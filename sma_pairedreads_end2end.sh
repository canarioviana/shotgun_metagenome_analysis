...
if [ -f "ref_genomes_ids.tsv" ]; then
    if [ ! -s "ref_genomes_ids.tsv" ]; then
        echo "✗  ERROR: Metadata file for reference genomes from GenBank 'ref_genomes_ids.tsv' exists but is empty!" | tee -a 0_workflow_progress.txt
        exit 1
    fi
    echo "✔  Metadata file for reference genomes from GenBank 'ref_genomes_ids.tsv' found and non-empty." | tee -a 0_workflow_progress.txt
    # Remove Windows CRLF line endings
    sed -i 's/\r$//' ref_genomes_ids.tsv
else
    echo "⚠️  Metadata file for reference genomes from GenBank 'ref_genomes_ids.tsv' not found. Reference genomes will not be downloaded." | tee -a 0_workflow_progress.txt
fi

# Check if sample metadata file 'metagenomes.tsv' exists and is not empty
if [ ! -s "metagenomes.tsv" ]; then
    echo "✗  ERROR: Metadata file for samples 'metagenomes.tsv' not found or is empty!" | tee -a 0_workflow_progress.txt
    exit 1
else
    echo "✔  Metadata file for samples 'metagenomes.tsv' found and non-empty." | tee -a 0_workflow_progress.txt
    sed -i 's/\r$//' metagenomes.tsv
fi

# Validate consistency between metagenomes.tsv and ref_genomes_ids.tsv
has_host_samples=0
missing_hosts=0
...
while IFS=$'\t' read -r sample ref_accession ref_name isolation_source others; do
    ...
    ref_clean="${ref_name,,}"
    # If the sample has an associated host
    if [[ -n "$ref_clean" && ! "$ref_clean" =~ ^(na|none|-)$ ]]; then
        has_host_samples=1

        # If the reference genomes file does not exist or is empty OR if the host is not listed in it
        if [ ! -s "ref_genomes_ids.tsv" ]; then
            echo "✗  ERROR: Sample '${sample}' requires host '${ref_name}' according to 'metagenomes.tsv', but 'ref_genomes_ids.tsv' does not exist or is empty!" | tee -a 0_workflow_progress.txt
            missing_hosts=$((missing_hosts + 1))
        elif ! awk -F'\t' -v ref="$ref_name" '$2 == ref {found=1} END {exit !found}' ref_genomes_ids.tsv; then
            echo "✗  ERROR: Sample '${sample}' requires host '${ref_name}' according to 'metagenomes.tsv', but this host was not found in 'ref_genomes_ids.tsv'!" | tee -a 0_workflow_progress.txt
            missing_hosts=$((missing_hosts + 1))
        fi
    fi
done < metagenomes.tsv

if [ "$missing_hosts" -gt 0 ]; then
    echo "✗  ERROR: Found $missing_hosts host inconsistency(ies). Please fix the metadata table before proceeding." | tee -a 0_workflow_progress.txt
    exit 1
fi

if [ "$has_host_samples" -eq 0 ]; then
    echo "⚠️  No host-associated samples were found in 'metagenomes.tsv'. Decontamination step will be skipped." | tee -a 0_workflow_progress.txt
fi
...
# Run fasterq-dump and file processing inside a subshell
            (
                cd 1_reads || exit 1
                ...
                # Check if the reads are single-end
                    ...
                    echo "Warning: no valid output produced for sample ${sample} (accession: ${accession}). It will be retried on the next run."
...
while IFS=$'\t' read -r sample ref_accession ref_name isolation_source others; do
    ...
    if [ ! -f "1_reads/${sample}_1.fq.gz" ] || [ ! -f "1_reads/${sample}_2.fq.gz" ]; then
        echo "✗  ERROR: Sample '${sample}' listed in metagenomes.tsv has no matching read files (expected 1_reads/${sample}_1.fq.gz and 1_reads/${sample}_2.fq.gz)." | tee -a 0_workflow_progress.txt
        missing_reads=$((missing_reads + 1))
    fi
done < metagenomes.tsv

if [ "$missing_reads" -gt 0 ]; then
    echo "✗  ERROR: Found $missing_reads sample(s) in metagenomes.tsv without matching read files in 1_reads/. Please fix the sample names in metagenomes.tsv or the read file names before proceeding." | tee -a 0_workflow_progress.txt
    exit 1
fi
...
if [ -f "2_fastqc.tar.gz" ] && [ -f "2_fastqc.tar.gz.md5" ] && md5sum -c "2_fastqc.tar.gz.md5" >/dev/null 2>&1; then
    ...
    reads_files=(1_reads/*.gz)
    if [ ${#reads_files[@]} -eq 0 ]; then
        echo "✗  ERROR: No .gz files found in 1_reads/ for FastQC." | tee -a 0_workflow_progress.txt
        exit 1
    fi
    ...
    # Run main software
    fastqc -t $(nproc --ignore=1) "${reads_files[@]}" -o 2_fastqc
    # Deactivate Conda environment
    conda deactivate
...
# Track whether this step actually (re)processed any sample, so the compression/checksum
...
for r1 in "${r1_files[@]}"; do
    ...
    r1filename=${r1##*/}
    # Extract sample name (using known suffix instead of first-underscore split)
    ...
    # A sample will actually be (re)processed
    work_done=true
...
if [ -f "4_fastqc.tar.gz" ] && [ -f "4_fastqc.tar.gz.md5" ] && md5sum -c "4_fastqc.tar.gz.md5" >/dev/null 2>&1; then
    ...
    # Guard against empty input
    reads_files=(3_fastp/*.gz)
    if [ ${#reads_files[@]} -eq 0 ]; then
        echo "✗  ERROR: No .gz files found in 3_fastp/ for FastQC." | tee -a 0_workflow_progress.txt
        exit 1
    fi
    ...
    conda activate fastqc
    # Run main software
    fastqc -t 4 "${reads_files[@]}" -o 4_fastqc
    # Deactivate Conda environment
    conda deactivate
...
# Check if ref_genomes_ids.tsv exists before running the step
if [ ! -f "ref_genomes_ids.tsv" ]; then
    echo "⚠️  ${workflow_step}: Reference genome(s) metadata file 'ref_genomes_ids.tsv' not found. Skipping reference genome(s) download." | tee -a 0_workflow_progress.txt
    echo "⚠️  The workflow will assume that all samples are not host-associated." | tee -a 0_workflow_progress.txt
else
    # Remove Windows CRLF line endings in-place
    sed -i 's/\r$//' ref_genomes_ids.tsv
    ...
    if [ "$all_present" -eq 1 ]; then
        ...
        # Generate checksum files for the reference genomes
...
if [ -f "5_bwa_index.md5" ] && md5sum -c "5_bwa_index.md5" >/dev/null 2>&1; then
    ...
    # Check if ref_genomes_ids.tsv exists before proceeding
    if [ ! -f "ref_genomes_ids.tsv" ]; then
        echo "⚠️  ${workflow_step}: Reference genome(s) metadata file 'ref_genomes_ids.tsv' not found. Skipping index(es) creation." | tee -a 0_workflow_progress.txt
        echo "⚠️  The workflow will assume that all samples are not host-associated" | tee -a 0_workflow_progress.txt
    else
        # Remove Windows CRLF line endings in-place
        sed -i 's/\r$//' ref_genomes_ids.tsv
        ...
        while IFS=$'\t' read -r ref_accession ref_name others; do
            ...
            # Inform current sample
            echo "▶  ${workflow_step} - ${ref_name} @ $(date +'%Y-%m-%d %H:%M:%S')" | tee -a 0_workflow_progress.txt
            # Start counting the running time
            loop_start_time=$SECONDS
            ...
            # Run main software (bwa-mem2 reads gzip-compressed references natively)
            bwa-mem2 index \
                -p "5_bwa_index/${ref_name}/${ref_name}" \
                "5_ref_genomes/${ref_accession}.fasta.gz"
            ...
            loop_running_time=$(printf "%02d:%02d:%02d" "$loop_hours" "$loop_minutes" "$loop_seconds")
            # Show the running time
            echo "✔  $workflow_step - $ref_name @ $(date +'%Y-%m-%d %H:%M:%S') - Total: ${loop_running_time} " | tee -a 0_workflow_progress.txt
...
# Check if metagenomes.tsv exists
if [ ! -f "metagenomes.tsv" ]; then
    echo "✗  ${workflow_step}: File 'metagenomes.tsv' not found!" | tee -a 0_workflow_progress.txt
    exit 1
fi
...
# Calculate sample size
i=1
sample_count=$(awk 'END {print NR}' metagenomes.tsv)

# Activate conda environment
conda activate bwa-mem2
# Loop through a list of file lines
tr -d '\r' < metagenomes.tsv |  awk '1' | \
while IFS=$'\t' read -r sample ref_accession ref_name isolation_source others; do
    ...
    if [ ! -f "$r1_input_file" ] || [ ! -f "$r2_input_file" ]; then
        echo "✗  ${workflow_step}: Trimmed reads not found for sample '${sample}' (expected ${r1_input_file} and ${r2_input_file}). Check that the 'sample' name in metagenomes.tsv matches the fastp output file name for this sample. Aborting workflow." | tee -a 0_workflow_progress.txt
        exit 1
    fi

    # Output files are the same regardless of whether the sample is host-associated
    r1_output_file="5_bwa_reads/${sample}_trimmed_nohost_1.fq.gz"
    r2_output_file="5_bwa_reads/${sample}_trimmed_nohost_2.fq.gz"

    # Check if the sample is host-associated
    ref_clean="${ref_name,,}"
    # Check if empty OR equal to na, none, or -
    if [[ -z "$ref_clean" || "$ref_clean" =~ ^(na|none|-)$ ]]; then
        # Skip if the symlinks already exist and resolve to a valid target
        if [ -e "$r1_output_file" ] && [ -e "$r2_output_file" ]; then
            echo "${workflow_step}: ${sample} is not host-associated and symlinks already exist. Skipping sample." | tee -a 0_workflow_progress.txt
            i=$((i + 1))
            continue
        fi
        # Remove any leftover broken/partial links before relinking
        rm -f "$r1_output_file" "$r2_output_file"
        # Paths are relative to 5_bwa_reads/, so go up one level to reach 3_fastp/
        ln -s "../${r1_input_file}" "$r1_output_file"
        ln -s "../${r2_input_file}" "$r2_output_file"
        i=$((i + 1))
        continue
    fi
    ...
    if [ ! -f "${index_prefix}.bwt.2bit.64" ]; then
        echo "✗  ${workflow_step}: Index not found for reference: ${ref_name} (${index_prefix}.bwt.2bit.64). Aborting workflow." | tee -a 0_workflow_progress.txt
        exit 1
    fi
    ...
    if [ -f "$r1_output_file" ] && [ -f "$r2_output_file" ] \
        ...
        r2_lines=$(zcat "$r2_output_file" | wc -l)
        if [ "$r1_lines" -gt 0 ] && [ $((r1_lines % 4)) -eq 0 ] && [ "$r1_lines" -eq "$r2_lines" ] \
            && ! zcat "$r1_output_file" | awk 'NR%4==0' | LC_ALL=C grep -qP '[^\x21-\x7e]' \
            && ! zcat "$r2_output_file" | awk 'NR%4==0' | LC_ALL=C grep -qP '[^\x21-\x7e]'; then
            echo "${workflow_step} output file already exists and is valid for sample: $sample. Skipping sample."
            i=$((i + 1))
            continue
        else
            echo "${workflow_step} found corrupted output for sample: $sample (passed gzip integrity but failed FASTQ content validation). Removing and reprocessing." | tee -a 0_workflow_progress.txt
            rm -f "$r1_output_file" "$r2_output_file" \
        ...
        echo "${workflow_step} found incomplete/corrupted output for sample: $sample. Removing partial files and reprocessing." | tee -a 0_workflow_progress.txt
        rm -f "$r1_output_file" "$r2_output_file" \
            "5_bwa_reads/${sample}_alignment.log" \
            "5_bwa_reads/${sample}_nofilter_flagstat.txt" \
            "5_bwa_reads/${sample}_hostfilter_flagstat.txt" \
    ...
    bwa-mem2 mem \
        ...
        | samtools sort -n -T "$sort_tmp_prefix" -@ $(nproc --ignore=1) - \
        | samtools fastq -c 6 \
    ...
    if [ -f "$r2_output_file" ] && ! gzip -t "$r2_output_file" 2>> 5_bwa_reads_corrupted.txt; then
        echo "5) Bwa-mem2 R2 output failed gzip integrity check for sample: $sample" | tee -a 0_workflow_progress.txt
        rm -f "$r1_output_file" "$r2_output_file"
    fi
    if [ -f "$r1_output_file" ] && [ -f "$r2_output_file" ]; then
        echo "Validating FASTQ content (read counts and quality bytes)"
        r1_lines=$(zcat "$r1_output_file" | wc -l)
        r2_lines=$(zcat "$r2_output_file" | wc -l)
        if [ "$r1_lines" -eq 0 ] || [ $((r1_lines % 4)) -ne 0 ] || [ "$r1_lines" -ne "$r2_lines" ]; then
            echo "5) Bwa-mem2 output has inconsistent/incomplete read counts for sample: $sample (R1 lines: ${r1_lines}, R2 lines: ${r2_lines}). Removing." | tee -a 0_workflow_progress.txt
            rm -f "$r1_output_file" "$r2_output_file"
        elif zcat "$r1_output_file" | awk 'NR%4==0' | LC_ALL=C grep -qP '[^\x21-\x7e]' \
            || zcat "$r2_output_file" | awk 'NR%4==0' | LC_ALL=C grep -qP '[^\x21-\x7e]'; then
            echo "5) Bwa-mem2 output has invalid quality byte(s) outside the printable ASCII range for sample: $sample. Removing." | tee -a 0_workflow_progress.txt
            rm -f "$r1_output_file" "$r2_output_file"
        fi
    fi
...
if [ -f "5_bwa_reads_fastqc.tar.gz" ] && [ -f "5_bwa_reads_fastqc.tar.gz.md5" ] && md5sum -c "5_bwa_reads_fastqc.tar.gz.md5" >/dev/null 2>&1; then
    ...
    # Guard against empty input
    reads_files=(5_bwa_reads/*.gz)
    if [ ${#reads_files[@]} -eq 0 ]; then
        echo "✗  ERROR: No .gz files found in 5_bwa_reads/ for FastQC." | tee -a 0_workflow_progress.txt
        exit 1
    fi
    ...
    conda activate fastqc
    # Run main software
    fastqc -t 4 "${reads_files[@]}" -o 5_bwa_reads_fastqc
    # Deactivate Conda environment
    conda deactivate
...
if [ -f "5_bwa_reads_fastqc_multiqc.tar.gz" ] && [ -f "5_bwa_reads_fastqc_multiqc.tar.gz.md5" ] && md5sum -c "5_bwa_reads_fastqc_multiqc.tar.gz.md5" >/dev/null 2>&1; then
    ...
    # Guard against an empty glob before calling multiqc with no input
    fastqc_zips=(5_bwa_reads_fastqc/*_fastqc.zip)
    if [ ${#fastqc_zips[@]} -eq 0 ]; then
        echo "✗  ERROR: No *_fastqc.zip files found in 5_bwa_reads_fastqc/ for MultiQC." | tee -a 0_workflow_progress.txt
        exit 1
    fi
    ...
    conda activate multiqc
    # Run main software
    multiqc "${fastqc_zips[@]}" -o 5_bwa_reads_fastqc_multiqc
    # Deactivate Conda environment
    conda deactivate
...
sample_count=${#r1_files[@]}
if [ "$sample_count" -eq 0 ]; then
    echo "✗  ${workflow_step}: No input found matching 5_bwa_reads/*_1.fq.gz. Check that the previous step completed successfully." | tee -a 0_workflow_progress.txt
    exit 1
fi

# Track whether this step actually (re)processed any sample, so the checksum/compression
# block below can be skipped entirely when nothing changed from a previous run
work_done=false
...
for r1 in "${r1_files[@]}"; do
    ...
    filename=${r1##*/}
    # Extract sample name
    sample=${filename%_trimmed_nohost_1.fq.gz}
...
compressed_file="6_kraken_report.tar.gz"
itens_to_compress=(6_kraken_report)

# Skip re-checksumming/recompressing/re-verifying if nothing changed and a valid
# archive from a previous run already exists
if [ "$work_done" = false ] && [ -f "$compressed_file" ] && [ -f "${compressed_file}.md5" ] \
    && md5sum -c "${compressed_file}.md5" >/dev/null 2>&1; then
    echo "✔  ${workflow_step} already completed successfully (${compressed_file} verified). Skipping compression." | tee -a 0_workflow_progress.txt
else
    ...
    # Check archive integrity (verifies the tar.gz is not corrupted / can be fully read)
    echo "${workflow_step}: Checking archive integrity" | tee -a 0_workflow_progress.txt
    if tar -tzf "${compressed_file}" > /dev/null; then
        echo "${compressed_file}: OK (valid tar.gz archive)" | tee -a 0_workflow_progress.txt
    else
        echo "${compressed_file}: FAILED (corrupted tar.gz archive)" | tee -a 0_workflow_progress.txt
    fi
fi
...
# Necessary because the script deletes 6_kraken_report/ at the end, once Bracken is done with it.
if [ -f "6_bracken_report.tar.gz" ] && [ -f "6_bracken_report.tar.gz.md5" ] && md5sum -c "6_bracken_report.tar.gz.md5" >/dev/null 2>&1; then
    echo "✔  ${workflow_step} already completed successfully (6_bracken_report.tar.gz verified). Skipping step." | tee -a 0_workflow_progress.txt
else
...
if [ "$sample_count" -eq 0 ]; then
    echo "✗  ${workflow_step}: No input found matching 6_kraken_report/*_kreport.tsv. Check that the previous step completed successfully." | tee -a 0_workflow_progress.txt
    exit 1
fi
...
# Delete the raw Kraken report directory now that Bracken is done reading from it
# (nothing later in the pipeline reads 6_kraken_report/ again; 6_kraken_report.tar.gz keeps the data)
echo "${workflow_step}: Deleting 6_kraken_report (kept as 6_kraken_report.tar.gz)" | tee -a 0_workflow_progress.txt
rm -r 6_kraken_report
...
if [ -f "6_bracken_comparison.tar.gz" ] && [ -f "6_bracken_comparison.tar.gz.md5" ] && md5sum -c "6_bracken_comparison.tar.gz.md5" >/dev/null 2>&1; then
    ...
    # Guard against empty input
    bracken_files=(6_bracken_report/*.bracken)
    if [ ${#bracken_files[@]} -eq 0 ]; then
        echo "✗  ERROR: ${workflow_step}: No .bracken files found in 6_bracken_report/. Check that the previous step completed successfully." | tee -a 0_workflow_progress.txt
        exit 1
    fi
...
if [ -f "6_bracken_krona.tar.gz" ] && [ -f "6_bracken_krona.tar.gz.md5" ] && md5sum -c "6_bracken_krona.tar.gz.md5" >/dev/null 2>&1; then
    ...
    sample_count=${#reports[@]}
    if [ "$sample_count" -eq 0 ]; then
        echo "✗  ${workflow_step}: No input found matching 6_bracken_report/*_breport.tsv. Check that the previous step completed successfully." | tee -a 0_workflow_progress.txt
        exit 1
    fi
...
if [ -f "6_metaphlan_comparison.tar.gz" ] && [ -f "6_metaphlan_comparison.tar.gz.md5" ] && md5sum -c "6_metaphlan_comparison.tar.gz.md5" >/dev/null 2>&1; then
    ...
    # Guard against empty input
    mprofile_files=(6_metaphlan/*_mprofile.txt)
    if [ ${#mprofile_files[@]} -eq 0 ]; then
        echo "✗  ERROR: ${workflow_step}: No *_mprofile.txt files found in 6_metaphlan/. Check that the previous step completed successfully." | tee -a 0_workflow_progress.txt
        exit 1
    fi
    ...
    # Activate Conda environment
    conda activate metaphlan
    merge_metaphlan_tables.py "${mprofile_files[@]}" \
...
# Track whether this step actually (re)assembled any sample, so the compression/checksum
# block below can be skipped entirely when nothing changed from a previous run
work_done=false
...
for r1 in "${r1_files[@]}"; do
    ...
    # Skip sample if output file already exists and looks complete (valid gzip + log present)
    output_file="7_megahit/${sample}_megahit.fasta"
    output_gz="${output_file}.gz"
    log_file="7_megahit/${sample}_megahit.log"
    if [ -f "$output_gz" ] && gzip -t "$output_gz" 2>/dev/null && [ -f "$log_file" ]; then
        echo "7) MEGAHIT output file already exists and is valid for sample: $sample ($output_gz). Skipping assembly."
        i=$((i + 1))
        continue
    elif [ -f "$output_file" ] || [ -f "$output_gz" ] || [ -f "$log_file" ]; then
        echo "7) MEGAHIT found incomplete/corrupted output for sample: $sample. Removing partial files and reprocessing."
        rm -f "$output_file" "$output_gz" "${output_gz}.md5" "$log_file"
    fi

    # A sample will actually be (re)assembled
    work_done=true
    ...
    # Compress the assembly (always kept compressed at rest; downstream steps read .fasta.gz)
    pigz -p $(nproc --ignore=1) "$output_file"
...
if [ "$work_done" = false ] && [ -f "$compressed_file" ] && [ -f "${compressed_file}.md5" ] \
    ...
    # Generate checksum files for the compressed assemblies (skip files already verified against an existing .md5)
    (cd 7_megahit && for file in *.fasta.gz; do
        [ -f "${file}.md5" ] && md5sum -c "${file}.md5" >/dev/null 2>&1 && continue
        echo "Processing checksum of file: ${file}"
        md5sum ${file} > ${file}.md5
    done) | tee -a 0_workflow_progress.txt
    echo "${workflow_step}: Checking file integrity" | tee -a 0_workflow_progress.txt
    (cd 7_megahit && md5sum -c *.md5) | tee -a 0_workflow_progress.txt
...
if [ -f "7_megahit_quast.tar.gz" ] && [ -f "7_megahit_quast.tar.gz.md5" ] && md5sum -c "7_megahit_quast.tar.gz.md5" >/dev/null 2>&1; then
    ...
    #Loop
    fasta_files=(7_megahit/*.fasta.gz)
    if [ ${#fasta_files[@]} -eq 0 ]; then
        echo "✗  ${workflow_step}: No input found matching 7_megahit/*.fasta.gz. Check that the previous step completed successfully." | tee -a 0_workflow_progress.txt
        exit 1
    fi
    i=1
    ...
    for file in "${fasta_files[@]}"; do
        ...
        filename=${file##*/}
        # Extract sample name
        sample=${filename%_megahit.fasta.gz}
...
for file in "${fasta_files[@]}"; do
    ...
    # Run main software (aragorn cannot read gzip or stdin, decompress to a temp file first)
...
# Track whether this step actually (re)processed any sample
work_done=false
...
for file in "${files[@]}"; do
    ...
    # Decompress to a temporary file, matching the bin-level Pyrodigal step's convention
    tmp_fasta="8_pyrodigal/${sample}_pyrodigal/${sample}.fsa"
    zcat "$file" > "$tmp_fasta"
    ...
    # Delete the temporary decompressed fasta
    rm -f "$tmp_fasta"
...
if [ "$sample_count" -eq 0 ]; then
    echo "✗  ${workflow_step}: No input found matching 8_pyrodigal/*/*.faa. Check that the previous step completed successfully." | tee -a 0_workflow_progress.txt
    exit 1
fi
...
for file in "${faa_files[@]}"; do
    ...
    # AMRFinderPlus cannot read gzipped FASTA — decompress the MEGAHIT assembly to a temp file first
    tmp_fasta=$(mktemp --suffix=.fasta)
    zcat "7_megahit/${sample}_megahit.fasta.gz" > "$tmp_fasta"
...
############################################################
## 8.7) Pyrodigal -> VFDB (diamond)

# Avoid literal glob pattern
shopt -s nullglob

# Software name for tracking progress in 0_workflow_progress.txt
workflow_step="8) VFDB (diamond)"
# Update the file 0_workflow_progress.txt
echo "▶▶▶  ${workflow_step} started @ $(date +'%Y-%m-%d %H:%M:%S') ▶▶▶" | tee -a 0_workflow_progress.txt
# Start counting the running time
start_time=$SECONDS
...
for file in "${faa_files[@]}"; do
    ...
    # Skip sample if output files already exist and look complete
    header_tsv="8_pyrodigal_vfdb/${sample}_vfdb.tsv"
    no_header_tsv="8_pyrodigal_vfdb/${sample}_vfdb_no_header.tsv"

    if [ -f "$no_header_tsv" ] && [ -s "$header_tsv" ]; then
        echo "${workflow_step} output file already exists and is valid for sample: $sample ($header_tsv). Skipping sample."
        i=$((i + 1))
        continue
    elif [ -f "$no_header_tsv" ] || [ -f "$header_tsv" ]; then
        echo "${workflow_step} found incomplete/corrupted output for sample: $sample. Removing partial files and reprocessing."
        rm -f "$no_header_tsv" "$header_tsv"
    fi
...
# Build a single combined table with all samples (one header line only)
combined_file="8_pyrodigal_vfdb/vfdb_all.tsv"
if [ "$work_done" = true ] || [ ! -s "$combined_file" ]; then
    ...
    header_tsv_files=(8_pyrodigal_vfdb/*_vfdb.tsv)
    if [ "${#header_tsv_files[@]}" -eq 0 ]; then
        echo "✗  ${workflow_step}: No per-sample VFDB tables found to concatenate." | tee -a 0_workflow_progress.txt
        exit 1
    fi
    # Take the header from the first file, then append every file's data rows (skip their own header)
    head -n 1 "${header_tsv_files[0]}" > "$combined_file"
    for f in "${header_tsv_files[@]}"; do
        tail -n +2 "$f" >> "$combined_file"
    done
    # The combined file is new/changed, so force the archive below to be rebuilt too
    work_done=true
fi
...
if [ -f "9_mmseqs_input.tar.gz" ] && [ -f "9_mmseqs_input.tar.gz.md5" ] && md5sum -c "9_mmseqs_input.tar.gz.md5" >/dev/null 2>&1; then
    ...
    # List of samples to exclude from MMseqs2 clustering, if necessary
    exclude_samples=()
    ...
    if [ "$sample_count" -eq 0 ]; then
        echo "✗  ${workflow_step}: No input found matching 8_pyrodigal/*/*.ffn (after sample exclusion filter). Check that the previous step completed successfully." | tee -a 0_workflow_progress.txt
        exit 1
    fi
    ...
    if [ "$n_faa_exclusive" -gt 0 ]; then
        echo "⚠️  $n_faa_exclusive protein IDs have no matching nucleotide sequence (see all_samples_faa_ids_exclusive.txt)" | tee -a 0_workflow_progress.txt
    fi
    if [ "$n_ffn_exclusive" -gt 0 ]; then
        echo "⚠️  $n_ffn_exclusive nucleotide IDs have no matching protein sequence (see all_samples_ffn_ids_exclusive.txt)" | tee -a 0_workflow_progress.txt
    fi
...
if [ -f "9_mmseqs.tar.gz" ] && [ -f "9_mmseqs.tar.gz.md5" ] && md5sum -c "9_mmseqs.tar.gz.md5" >/dev/null 2>&1; then
    ...
    # Delete the raw MMseqs2 input directory now that 9_mmseqs has been generated from it
    # (nothing later in the pipeline reads 9_mmseqs_input/ again; 9_mmseqs_input.tar.gz keeps the data)
    echo "${workflow_step}: Deleting 9_mmseqs_input (kept as 9_mmseqs_input.tar.gz)" | tee -a 0_workflow_progress.txt
    rm -r 9_mmseqs_input
fi
...
if [ -f "9_mmseqs_amrfinder.tar.gz" ] && [ -f "9_mmseqs_amrfinder.tar.gz.md5" ] && md5sum -c "9_mmseqs_amrfinder.tar.gz.md5" >/dev/null 2>&1; then
    ...
    # Recover the raw non-redundant gene catalog protein file from its archive if a previous
    # successful run already deleted 9_mmseqs/ at the end of step 9
    if [ ! -s "9_mmseqs/all_samples_f1_nr.faa" ]; then
        echo "${workflow_step}: 9_mmseqs/all_samples_f1_nr.faa not found — attempting to extract it from 9_mmseqs.tar.gz" | tee -a 0_workflow_progress.txt
        if [ -f "9_mmseqs.tar.gz" ]; then
            tar -xzf 9_mmseqs.tar.gz
        fi
        if [ ! -s "9_mmseqs/all_samples_f1_nr.faa" ]; then
            echo "✗  ${workflow_step}: 9_mmseqs/all_samples_f1_nr.faa could not be found or recovered. Run the MMseqs2 easy-linclust step first." | tee -a 0_workflow_progress.txt
            exit 1
        fi
    fi
...
if [ -f "9_mmseqs_dbcan.tar.gz" ] && [ -f "9_mmseqs_dbcan.tar.gz.md5" ] && md5sum -c "9_mmseqs_dbcan.tar.gz.md5" >/dev/null 2>&1; then
    ...
    faa_check=(9_mmseqs_dbcan/tmp/*.faa)
    if [ ${#faa_check[@]} -eq 0 ]; then
        # Recover the raw non-redundant gene catalog protein file from its archive if a previous
        # successful run already deleted 9_mmseqs/ at the end of step 9
        if [ ! -s "9_mmseqs/all_samples_f1_nr.faa" ]; then
            echo "${workflow_step}: 9_mmseqs/all_samples_f1_nr.faa not found — attempting to extract it from 9_mmseqs.tar.gz" | tee -a 0_workflow_progress.txt
    ...
    sample_count=${#faa_files[@]}
    if [ "$sample_count" -eq 0 ]; then
        echo "✗  ${workflow_step}: No input found matching 9_mmseqs_dbcan/tmp/*.faa. Check that the previous step completed successfully." | tee -a 0_workflow_progress.txt
        exit 1
    fi
    ...
    for file in "${faa_files[@]}"; do
        ...
        # Inform current part
        echo "▶  ${workflow_step} — part ${sample} (${i}/${sample_count}) @ $(date +'%Y-%m-%d %H:%M:%S')" | tee -a 0_workflow_progress.txt
        # Start counting the running time
        loop_start_time=$SECONDS
        ...
        loop_running_time=$(printf "%02d:%02d:%02d" "$loop_hours" "$loop_minutes" "$loop_seconds")
        # Show the running time
        echo "✔  ${workflow_step} — part ${sample} (${i}/${sample_count}) @ $(date +'%Y-%m-%d %H:%M:%S') — Total: ${loop_running_time}" | tee -a 0_workflow_progress.txt
    ...
    for output_file in "${output_files[@]}"; do
        ...
        if (( ${#files[@]} )); then
            ...
            echo "⚠️  ${workflow_step}: No ${output_file} files found under 9_mmseqs_dbcan/tmp/*_dbcan/." | tee -a 0_workflow_progress.txt
        fi
    done
...
############################################################
## 9.6) MMseqs2 -> VFDB (diamond)

# Avoid literal glob pattern
shopt -s nullglob

# Software name for tracking progress in 0_workflow_progress.txt
workflow_step="9) MMseqs2 -> VFDB (diamond)"
# Update the file 0_workflow_progress.txt
echo "▶▶▶  ${workflow_step} started @ $(date +'%Y-%m-%d %H:%M:%S') ▶▶▶" | tee -a 0_workflow_progress.txt
# Start counting the running time
start_time=$SECONDS
...
# Define the single sample name based on the input file
sample="all_samples_f1_nr"

# Input file
input_faa="9_mmseqs/${sample}.faa"

# Recover the raw non-redundant gene catalog protein file from its archive if a previous
# successful run already deleted 9_mmseqs/ at the end of step 9
if [ ! -s "$input_faa" ]; then
    echo "${workflow_step}: ${input_faa} not found — attempting to extract it from 9_mmseqs.tar.gz" | tee -a 0_workflow_progress.txt
    if [ -f "9_mmseqs.tar.gz" ]; then
        tar -xzf 9_mmseqs.tar.gz
    fi
fi
...
if [ ! -s "$input_faa" ]; then
    echo "✗  ${workflow_step}: No input found matching ${input_faa}. Check that the previous step completed successfully." | tee -a 0_workflow_progress.txt
    exit 1
fi

# Output files
header_tsv="9_mmseqs_vfdb/${sample}_vfdb.tsv"
no_header_tsv="9_mmseqs_vfdb/${sample}_vfdb_no_header.tsv"

# Activate Conda environment
conda activate diamond
# Skip processing if output files already exist and look complete.
if [ -f "$no_header_tsv" ] && [ -s "$header_tsv" ]; then
    echo "${workflow_step} output file already exists and is valid for sample: $sample ($header_tsv). Skipping Diamond run."
else
    if [ -f "$no_header_tsv" ] || [ -f "$header_tsv" ]; then
        echo "${workflow_step} found incomplete/corrupted output for sample: $sample. Removing partial files and reprocessing."
        rm -f "$no_header_tsv" "$header_tsv"
    fi
    # Processing will actually happen
    work_done=true
    # Run main software
    diamond \
        blastp \
...
cp "$header_tsv" 9_mmseqs_vfdb.tsv

compressed_file="9_mmseqs_vfdb.tar.gz"
itens_to_compress=(9_mmseqs_vfdb 9_mmseqs_vfdb.tsv)
...
if [ -f "9_mmseqs_salmon_index.tar.gz" ] && [ -f "9_mmseqs_salmon_index.tar.gz.md5" ] && md5sum -c "9_mmseqs_salmon_index.tar.gz.md5" >/dev/null 2>&1; then
    ...
    # Recover the raw non-redundant gene catalog from its archive if a previous successful
    # run already deleted 9_mmseqs/ at the end of step 9
    if [ ! -s "9_mmseqs/all_samples_f1_nr.ffn" ]; then
        echo "${workflow_step}: 9_mmseqs/all_samples_f1_nr.ffn not found — attempting to extract it from 9_mmseqs.tar.gz" | tee -a 0_workflow_progress.txt
        if [ -f "9_mmseqs.tar.gz" ]; then
            tar -xzf 9_mmseqs.tar.gz
        fi
        if [ ! -s "9_mmseqs/all_samples_f1_nr.ffn" ]; then
            echo "✗  ${workflow_step}: 9_mmseqs/all_samples_f1_nr.ffn could not be found or recovered. Run the MMseqs2 easy-linclust step first." | tee -a 0_workflow_progress.txt
            exit 1
        fi
    fi
...
if [ -f "9_mmseqs_salmon_quant.tar.gz" ] && [ -f "9_mmseqs_salmon_quant.tar.gz.md5" ] && md5sum -c "9_mmseqs_salmon_quant.tar.gz.md5" >/dev/null 2>&1 \
    ...
    # Recover the raw salmon index from its archive if a previous successful run already
    # deleted 9_mmseqs_salmon_index/ (this step needs the raw index, not just the .tar.gz)
    if [ ! -d "9_mmseqs_salmon_index/nr_index" ] || [ -z "$(ls -A 9_mmseqs_salmon_index/nr_index 2>/dev/null)" ]; then
        echo "${workflow_step}: 9_mmseqs_salmon_index/ not found — attempting to extract it from 9_mmseqs_salmon_index.tar.gz" | tee -a 0_workflow_progress.txt
        if [ -f "9_mmseqs_salmon_index.tar.gz" ]; then
            tar -xzf 9_mmseqs_salmon_index.tar.gz
        fi
        if [ ! -d "9_mmseqs_salmon_index/nr_index" ] || [ -z "$(ls -A 9_mmseqs_salmon_index/nr_index 2>/dev/null)" ]; then
            echo "✗  ${workflow_step}: 9_mmseqs_salmon_index could not be found or recovered. Run the Salmon index step first." | tee -a 0_workflow_progress.txt
            exit 1
        fi
    fi
    ...
    if [ ${#sample_dirs[@]} -eq 0 ]; then
        echo "✗  ${workflow_step}: No input found matching 9_mmseqs_salmon_quant/*/. Check that the previous step completed successfully." | tee -a 0_workflow_progress.txt
        exit 1
    fi
    ...
    # Compress merged tables
    echo "${workflow_step}: Compressing merged Salmon tables" | tee -a 0_workflow_progress.txt
    pigz -p $(nproc --ignore=1) -f "${out_dir}/salmon_tpm.tsv" "${out_dir}/salmon_numreads.tsv" "${out_dir}/salmon_length.tsv"

    # Compress the merged output directory
    ...
    # Compress the quantification output directory
    ...
    # Delete the raw salmon index now that quantification has been generated from it
    echo "${workflow_step}: Deleting 9_mmseqs_salmon_index (kept as 9_mmseqs_salmon_index.tar.gz)" | tee -a 0_workflow_progress.txt
    rm -rf 9_mmseqs_salmon_index

    # Delete the raw MMseqs2 non-redundant catalog now that every step-9 substep that needs it
    # (AMRFinderPlus, dbCAN, eggNOG-mapper, Salmon index) has already run
    # (nothing later in the pipeline reads 9_mmseqs/ again; 9_mmseqs.tar.gz keeps the data)
    echo "${workflow_step}: Deleting 9_mmseqs (kept as 9_mmseqs.tar.gz)" | tee -a 0_workflow_progress.txt
    rm -rf 9_mmseqs
fi
...
# This session uses information from file metagenomes.tsv
...
# Calculate the sample count to display loop progress
i=1
fasta_files=(7_megahit/*_megahit.fasta.gz)
sample_count=${#fasta_files[@]}
if [ "$sample_count" -eq 0 ]; then
    echo "✗  ${workflow_step}: No input found matching 7_megahit/*_megahit.fasta.gz. Check that the previous step completed successfully." | tee -a 0_workflow_progress.txt
    exit 1
fi
...
while IFS=$'\t' read -r source sample_list; do
    ...
    if [ "$num_samples" -ge 2 ]; then
        ...
        loop_running_time=$(printf "%02d:%02d:%02d" "$loop_hours" "$loop_minutes" "$loop_seconds")
        # Show the running time
        echo "✔  ${workflow_step} — ${source} (${num_samples} samples) @ $(date +'%Y-%m-%d %H:%M:%S') — Total: ${loop_running_time}" | tee -a 0_workflow_progress.txt
    else
        echo "⚠️  ${workflow_step}: Skipping concatenation for ${source}. Only ${num_samples} sample(s) found (minimum 2 required)."
    fi
...
if [ ${#raw_seqkit_fastas[@]} -gt 0 ]; then
    echo "${workflow_step}: Deleting redundant plain .fasta copies in 10_seqkit/ (kept as .fasta.gz)" | tee -a 0_workflow_progress.txt
    rm -f "${raw_seqkit_fastas[@]}"
fi
if [ -d "10_seqkit" ] && ! { [ -f "10_seqkit.tar.gz" ] && [ -f "10_seqkit.tar.gz.md5" ] && md5sum -c "10_seqkit.tar.gz.md5" >/dev/null 2>&1; }; then
    seqkit_compressed_file="10_seqkit.tar.gz"
    seqkit_itens_to_compress=(10_seqkit)
    echo "${workflow_step}: Compressing 10_seqkit" | tee -a 0_workflow_progress.txt
    if ! tar -c --use-compress-program=pigz -f "${seqkit_compressed_file}" "${seqkit_itens_to_compress[@]}"; then
        echo "✗  ERROR: ${seqkit_compressed_file}: FAILED to create archive" | tee -a 0_workflow_progress.txt
        exit 1
    fi
    md5sum "${seqkit_compressed_file}" > "${seqkit_compressed_file}".md5
    echo "${workflow_step}: Checking file integrity of ${seqkit_compressed_file}" | tee -a 0_workflow_progress.txt
    if ! md5sum -c "${seqkit_compressed_file}".md5 | tee -a 0_workflow_progress.txt; then
        echo "✗  ERROR: ${seqkit_compressed_file}: integrity check failed" | tee -a 0_workflow_progress.txt
        exit 1
    fi
fi
...
# Recover 10_seqkit_concat from its archive if a previous successful run of this step
# already deleted it (needed here to (re)build any missing/corrupted .mmi index)
if [ -f "10_seqkit_concat.tar.gz" ] && { [ ! -d "10_seqkit_concat" ] || [ -z "$(ls -A 10_seqkit_concat 2>/dev/null)" ]; }; then
    echo "${workflow_step}: 10_seqkit_concat/ not found — attempting to extract it from 10_seqkit_concat.tar.gz" | tee -a 0_workflow_progress.txt
    tar -xzf 10_seqkit_concat.tar.gz
fi

# Track whether this step actually (re)built any index
work_done=false
...
while IFS=$'\t' read -r source sample_list; do
    ...
    # An index will actually be (re)built for this source
    work_done=true
    ...
    if [ "$num_samples" -ge 2 ]; then
        ...
        echo "⚠️  ${workflow_step}: Skipping index for ${source}. No sample found (minimum 1 required)."
    fi

done < <(tr -d '\r' < "$input_file" | awk 'BEGIN {FS="\t"}
{
    # Concatenate Sample ID ($1) for each Isolaton Source ($4)
    if (samples_by_source[$4] == "") {
        samples_by_source[$4] = $1;
...
# Compress and delete 10_seqkit_concat now that the minimap2 indexes have been built from it.
if [ -d "10_seqkit_concat" ]; then
    concat_compressed_file="10_seqkit_concat.tar.gz"
    concat_itens_to_compress=(10_seqkit_concat)
    echo "${workflow_step}: Compressing 10_seqkit_concat" | tee -a 0_workflow_progress.txt
    if ! tar -c --use-compress-program=pigz -f "${concat_compressed_file}" "${concat_itens_to_compress[@]}"; then
        echo "✗  ERROR: ${concat_compressed_file}: FAILED to create archive" | tee -a 0_workflow_progress.txt
        exit 1
    fi
    md5sum "${concat_compressed_file}" > "${concat_compressed_file}".md5
    echo "${workflow_step}: Checking file integrity of ${concat_compressed_file}" | tee -a 0_workflow_progress.txt
    if md5sum -c "${concat_compressed_file}".md5 | tee -a 0_workflow_progress.txt; then
        echo "${workflow_step}: Deleting 10_seqkit_concat (kept as ${concat_compressed_file})" | tee -a 0_workflow_progress.txt
        rm -r 10_seqkit_concat
    else
        echo "⚠️  ${workflow_step}: ${concat_compressed_file} failed verification. Keeping raw 10_seqkit_concat/ for safety." | tee -a 0_workflow_progress.txt
    fi
fi
...
# Recover 10_seqkit_concat from its archive if step 10 already compressed/deleted it
if [ -f "10_seqkit_concat.tar.gz" ] && { [ ! -d "10_seqkit_concat" ] || [ -z "$(ls -A 10_seqkit_concat 2>/dev/null)" ]; }; then
    echo "${workflow_step}: 10_seqkit_concat/ not found — attempting to extract it from 10_seqkit_concat.tar.gz" | tee -a 0_workflow_progress.txt
    tar -xzf 10_seqkit_concat.tar.gz
fi

# Track whether this step actually (re)binned any source
work_done=false
...
while IFS=$'\t' read -r source sample_list; do
    ...
    if [ "$num_samples" -ge 2 ]; then
        ...
        # A source will actually be (re)binned
        work_done=true
        ...
        echo "▶  ${workflow_step} — ${source} (${num_samples} sample: ${sample_list}) @ $(date +'%Y-%m-%d %H:%M:%S')" | tee -a 0_workflow_progress.txt
        # Start counting the running time
        loop_start_time=$SECONDS
        ...
        SemiBin2 single_easy_bin \
            --threads $(nproc --ignore=1) \
            --self-supervised \
            --input-fasta "10_seqkit/${sample_list}_megahit_filtered.fasta.gz" \
        ...
        echo "⚠️  ${workflow_step}: Skipping binning for ${source}. No sample found (minimum 1 required)."
...
# Skip regenerating the bin listing and re-archiving the whole directory if nothing
# changed and a valid archive from a previous run already exists
if [ "$work_done" = false ] && [ -f "$compressed_file" ] && [ -f "${compressed_file}.md5" ] \
    && md5sum -c "${compressed_file}.md5" >/dev/null 2>&1; then
    echo "✔  ${workflow_step} already completed successfully (${compressed_file} verified). Skipping archiving." | tee -a 0_workflow_progress.txt
else
    ...
    # Archive the output directory (kept uncompressed — the bins inside are already
    # individually gzipped, so an extra pigz pass here would be wasted effort)
    echo "Archiving output directory" | tee -a 0_workflow_progress.txt
    if ! tar -cf "${compressed_file}" "${itens_to_compress[@]}"; then
        echo "✗  ERROR: ${compressed_file}: FAILED to create archive" | tee -a 0_workflow_progress.txt
        exit 1
    fi
    # Generate checksum file of compressed directory file
...
if [ -f "12_checkm2.tar.gz" ] && [ -f "12_checkm2.tar.gz.md5" ] && md5sum -c "12_checkm2.tar.gz.md5" >/dev/null 2>&1; then
    ...
    if [ "$sample_count" -eq 0 ]; then
        echo "✗  ${workflow_step}: No input found matching 11_semibin/*_semibin/. Check that the previous step completed successfully." | tee -a 0_workflow_progress.txt
        exit 1
    fi
    ...
    if [ ${#files[@]} -gt 0 ]; then
        ...
        echo "⚠️  ${workflow_step}: No 12_checkm2/*_checkm2.tsv files found. checkm2_all.tsv was not created." | tee -a 0_workflow_progress.txt
    fi
...
if [ -f "12_gunc.tar.gz" ] && [ -f "12_gunc.tar.gz.md5" ] && md5sum -c "12_gunc.tar.gz.md5" >/dev/null 2>&1; then
    ...
    # Ensure 12_checkm2.tsv is present; recover it from the archive if missing
    if [ ! -f "12_checkm2.tsv" ]; then
        echo "${workflow_step}: 12_checkm2.tsv not found — attempting to extract it from 12_checkm2.tar.gz" | tee -a 0_workflow_progress.txt
        if [ -f "12_checkm2.tar.gz" ]; then
            tar -xzf 12_checkm2.tar.gz 12_checkm2.tsv
        fi
        if [ ! -f "12_checkm2.tsv" ]; then
            echo "✗  ${workflow_step}: 12_checkm2.tsv could not be found or recovered. Run CheckM2 first." | tee -a 0_workflow_progress.txt
            exit 1
        fi
    fi
    ...
    passed_bins_list="12_gunc/passed_qc_bins.txt"
    ...
    for dir in "${sample_dirs[@]}"; do
        ...
        # Build a filtered input directory with symlinks only to bins that passed the CheckM2 quality filter
        filtered_input_dir="12_gunc/${sample}_gunc_input"
        mkdir -p "$filtered_input_dir"
        for bin in 11_semibin/${sample}_semibin/*.fasta.gz; do
            [ -f "$bin" ] || continue
            ...
            if grep -qxF "$bin_filename" "$passed_bins_list" || grep -qxF "$bin_basename_raw" "$passed_bins_list"; then
                ln -sf "$(readlink -f "$bin")" "${filtered_input_dir}/${bin_filename}"
            fi
        done

        # Skip sample if no bins passed the CheckM2 quality filter
        if [ -z "$(ls -A "$filtered_input_dir" 2>/dev/null)" ]; then
            echo "${workflow_step}: no bins passed the CheckM2 quality filter for sample $sample. Skipping sample." | tee -a 0_workflow_progress.txt
            rm -rf "12_gunc/${sample}_gunc" "12_gunc/${sample}_gunc_temp" "$filtered_input_dir"
            i=$((i + 1))
            continue
        fi
        ...
        rm -r 12_gunc/${sample}_gunc 12_gunc/${sample}_gunc_temp "$filtered_input_dir"
    ...
    if [ ${#files[@]} -gt 0 ]; then
        ...
        echo "⚠️  ${workflow_step}: No 12_gunc/*_gunc.tsv files found. gunc_all.tsv was not created." | tee -a 0_workflow_progress.txt
    fi
...
if [ -f "12_gtdbtk.tar.gz" ] && [ -f "12_gtdbtk.tar.gz.md5" ] && md5sum -c "12_gtdbtk.tar.gz.md5" >/dev/null 2>&1; then
    ...
    # Inform the beginning of the workflow
    ...
    for map in "classify/gtdbtk.bac120.summary.tsv:12_gtdbtk_bacteria.tsv" \
               "classify/gtdbtk.ar53.summary.tsv:12_gtdbtk_archaea.tsv" \
               "gtdbtk.log:12_gtdbtk.log" ; do
    ...
    if [ -f "12_gtdbtk/12_gtdbtk_bacteria.tsv" ] || [ -f "12_gtdbtk/12_gtdbtk_archaea.tsv" ]; then
        first_file=""
        for f in "12_gtdbtk/12_gtdbtk_bacteria.tsv" "12_gtdbtk/12_gtdbtk_archaea.tsv"; do
            [ -f "$f" ] && first_file="$f" && break
        done
        head -n 1 "$first_file" > "12_gtdbtk/12_gtdbtk_archaea_bacteria.tsv"
        for f in "12_gtdbtk/12_gtdbtk_bacteria.tsv" "12_gtdbtk/12_gtdbtk_archaea.tsv"; do
            [ -f "$f" ] || continue
            tail -n +2 "$f" >> "12_gtdbtk/12_gtdbtk_archaea_bacteria.tsv"
        done
    fi

    # Split the combined summaries back into one file per sample
    for domain in bacteria archaea; do
        src="12_gtdbtk/12_gtdbtk_${domain}.tsv"
        [ -f "$src" ] || continue
    ...
    # Copy the combined results file to the main directory (quick access without extracting the archive)
    cp 12_gtdbtk/12_gtdbtk_archaea_bacteria.tsv 12_gtdbtk.tsv
    ...
    itens_to_compress=(12_gtdbtk 12_gtdbtk.tsv)
    echo "${workflow_step}: Compressing output directory" | tee -a 0_workflow_progress.txt
    if ! tar -c --use-compress-program=pigz -f "${compressed_file}" "${itens_to_compress[@]}"; then
        echo "✗  ERROR: ${compressed_file}: FAILED to create archive" | tee -a 0_workflow_progress.txt
        exit 1
...
if [ -f "12_quast.tar.gz" ] && [ -f "12_quast.tar.gz.md5" ] && md5sum -c "12_quast.tar.gz.md5" >/dev/null 2>&1; then
    ...
    # Completeness >= 70%, Contamination <= 5%, Contig_N50 >= 5000 bp
    echo "${workflow_step}: Filtering genomes from 12_checkm2.tsv (Completeness >= 70%, Contamination <= 5%, N50 >= 5000 bp)" | tee -a 0_workflow_progress.txt
    passed_bins_list="12_quast/passed_qc_bins.txt"
    awk -F'\t' 'NR>1 { if ($2 >= 70 && $3 <= 5 && $7 >= 5000) print $1 }' 12_checkm2.tsv > "$passed_bins_list"
    ...
    for dir in "${sample_dirs[@]}"; do
        ...
        # Build a list of bin fasta files that passed the CheckM2 quality filter
        filtered_bins=()
        for bin in 11_semibin/${sample}_semibin/*.fasta.gz; do
            [ -f "$bin" ] || continue
            bin_filename=$(basename "$bin")
...
if [ -f "13_aragorn.tar.gz" ] && [ -f "13_aragorn.tar.gz.md5" ] && md5sum -c "13_aragorn.tar.gz.md5" >/dev/null 2>&1; then
    ...
    # Completeness >= 70%, Contamination <= 5%, Contig_N50 >= 5000 bp
    echo "${workflow_step}: Filtering genomes from 12_checkm2.tsv (Completeness >= 70%, Contamination <= 5%, N50 >= 5000 bp)" | tee -a 0_workflow_progress.txt
    passed_bins_list="13_aragorn/passed_qc_bins.txt"
    awk -F'\t' 'NR>1 { if ($2 >= 70 && $3 <= 5 && $7 >= 5000) print $1 }' 12_checkm2.tsv > "$passed_bins_list"
    ...
    for dir in "${sample_dirs[@]}"; do
        ...
        for file in 11_semibin/${sample}_semibin/*.fasta.gz; do
            ...
            # Skip bin if it did not pass the CheckM2 quality filter
            bin_filename=$(basename "$file")
            bin_basename_raw="${bin_filename%.gz}"
            if ! grep -qxF "$bin_filename" "$passed_bins_list" && ! grep -qxF "$bin_basename_raw" "$passed_bins_list"; then
                continue
            fi
            ...
            # Run software (aragorn cannot read gzip or stdin, decompress to a temp file first)
            tmp_fasta=$(mktemp --suffix=.fasta)
            zcat "$file" > "$tmp_fasta"
            aragorn \
                "$tmp_fasta" \
...
if [ -f "13_pybarrnap.tar.gz" ] && [ -f "13_pybarrnap.tar.gz.md5" ] && md5sum -c "13_pybarrnap.tar.gz.md5" >/dev/null 2>&1; then
    ...
    # Completeness >= 70%, Contamination <= 5%, Contig_N50 >= 5000 bp
    echo "${workflow_step}: Filtering genomes from 12_checkm2.tsv (Completeness >= 70%, Contamination <= 5%, N50 >= 5000 bp)" | tee -a 0_workflow_progress.txt
    passed_bins_list="13_pybarrnap/passed_qc_bins.txt"
    awk -F'\t' 'NR>1 { if ($2 >= 70 && $3 <= 5 && $7 >= 5000) print $1 }' 12_checkm2.tsv > "$passed_bins_list"
...
# Completeness >= 70%, Contamination <= 5%, Contig_N50 >= 5000 bp
echo "${workflow_step}: Filtering genomes from 12_checkm2.tsv (Completeness >= 70%, Contamination <= 5%, N50 >= 5000 bp)" | tee -a 0_workflow_progress.txt
passed_bins_list="13_pyrodigal/passed_qc_bins.txt"
awk -F'\t' 'NR>1 { if ($2 >= 70 && $3 <= 5 && $7 >= 5000) print $1 }' 12_checkm2.tsv > "$passed_bins_list"
...
for dir in "${sample_dirs[@]}"; do
    ...
    for file in 11_semibin/${sample}_semibin/*.fasta.gz; do
        ...
        # Skip bin if it did not pass the CheckM2 quality filter
        if ! grep -qxF "$filename" "$passed_bins_list" && ! grep -qxF "${filename%.gz}" "$passed_bins_list"; then
            continue
        fi
        ...
        # A bin will actually be (re)processed
        work_done=true
...
sample_count=${#sample_dirs[@]}
if [ "$sample_count" -eq 0 ]; then
    echo "✗  ${workflow_step}: No input found matching 13_pyrodigal/*_pyrodigal/. Check that the previous step completed successfully." | tee -a 0_workflow_progress.txt
    exit 1
fi
...
############################################################
## 13.7) Pyrodigal -> VFDB (diamond)

# Avoid literal glob pattern
shopt -s nullglob

# Software name for tracking progress in 0_workflow_progress.txt
workflow_step="13) VFDB (diamond)"
# Update the file 0_workflow_progress.txt
echo "▶▶▶  ${workflow_step} started @ $(date +'%Y-%m-%d %H:%M:%S') ▶▶▶" | tee -a 0_workflow_progress.txt
# Start counting the running time
start_time=$SECONDS
...
if [ "$work_done" = true ] || [ ! -s "$combined_file" ]; then
    echo "${workflow_step}: Building combined table ${combined_file}" | tee -a 0_workflow_progress.txt
    header_tsv_files=(13_pyrodigal_vfdb/*/*/*_vfdb.tsv)
    if [ "${#header_tsv_files[@]}" -eq 0 ]; then
        echo "✗  ${workflow_step}: No per-bin VFDB tables found to concatenate." | tee -a 0_workflow_progress.txt
        exit 1
    fi
    # Take the header from the first file, then append every file's data rows (skip their own header)
    head -n 1 "${header_tsv_files[0]}" > "$combined_file"
...
# Copy and rename the output file
cp 13_pyrodigal_vfdb/vfdb_all.tsv 13_pyrodigal_vfdb.tsv

# Compress the output directory
compressed_file="13_pyrodigal_vfdb.tar.gz"
itens_to_compress=(13_pyrodigal_vfdb 13_pyrodigal_vfdb.tsv)
...
sample_count=${#sample_dirs[@]}
if [ "$sample_count" -eq 0 ]; then
    echo "✗  ${workflow_step}: No input found matching 13_pyrodigal/*/. Check that the previous step completed successfully." | tee -a 0_workflow_progress.txt
    exit 1
fi
